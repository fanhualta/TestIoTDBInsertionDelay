...
//    session.close();
  ...
  private static void insert(String deviceId, int beginTag) throws IoTDBSessionException {
    ...
    TSStatus status = session.insert(deviceId, System.currentTimeMillis(), measurements, values);
    LOG.info("Device(SG){}在time={}时刻写10w个数据点花费：{}ms, 分耗时为{}", deviceId, begin,
        (System.nanoTime() - begin) / 1_000_000, status.getStatusType().getMessage());
  }
